...
class MultiWorkerAssignmentTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.worker1 = User.objects.create_user(
            username="worker1", password="worker123", email="worker1@test.com"
        )
        UserProfile.objects.create(
            user=self.worker1,
            identity="worker",
        ...
        self.worker2 = User.objects.create_user(
            username="worker2", password="worker123", email="worker2@test.com"
        )
        UserProfile.objects.create(
            user=self.worker2,
            identity="worker",
        ...
        self.worker3 = User.objects.create_user(
            username="worker3", password="worker123", email="worker3@test.com"
        )
        UserProfile.objects.create(
            user=self.worker3,
            identity="worker",
        ...
        self.customer = User.objects.create_user(
            username="customer1", password="customer123", email="customer@test.com"
        )
        UserProfile.objects.create(
            user=self.customer, identity="customer", phoneNumber="13800000005"
        )
    ...
    def test_single_worker_assignment(self):
        ...
        response = self.client.post(
            ...
            data=json.dumps({"reportId": self.report.id, "workerName": "worker1"}),
        ...
        assignments = OrderAssignment.objects.filter(
            report=self.report, status="active"
        )
    ...
    def test_multi_worker_assignment(self):
        ...
        self.assertEqual(data["message"], "Success")
        self.assertEqual(
            data["data"]["assignedWorkers"], ["worker1", "worker2", "worker3"]
        )
    ...
    def test_max_workers_exceeded(self):
        ...
        UserProfile.objects.create(
            user=worker4, identity="worker", phoneNumber="13800000006"
        )

        worker5 = User.objects.create_user(username="worker5", password="worker123")
        UserProfile.objects.create(
            user=worker5, identity="worker", phoneNumber="13800000007"
        )

        worker6 = User.objects.create_user(username="worker6", password="worker123")
        UserProfile.objects.create(
            user=worker6, identity="worker", phoneNumber="13800000008"
        )
    ...
    def test_invalid_worker(self):
        ...
        response = self.client.post(
            "/api/dashboard/assign_order/",
            data=json.dumps(
                {"reportId": self.report.id, "workerNames": ["non_existent_worker"]}
            ),
            content_type="application/json",
        )
    ...
    def test_duplicate_assignment(self):
        ...
        self.client.post(
            "/api/dashboard/assign_order/",
            data=json.dumps({"reportId": self.report.id, "workerNames": ["worker1"]}),
            content_type="application/json",
        )
        ...
        response = self.client.post(
            "/api/dashboard/assign_order/",
            data=json.dumps({"reportId": report2.id, "workerNames": ["worker1"]}),
            content_type="application/json",
        )
    ...
    def test_get_report_with_multi_workers(self):
        ...
        self.client.post(
            "/api/dashboard/assign_order/",
            data=json.dumps(
                {"reportId": self.report.id, "workerNames": ["worker1", "worker2"]}
            ),
            content_type="application/json",
        )
        ...
        target_report = next(
            (r for r in reports if r["reportId"] == self.report.id), None
        )
    ...
    def test_complete_report_updates_assignment_status(self):
        ...
        response = self.client.post(
            ...
            data=json.dumps({"reportId": self.report.id}),
            content_type="application/json",
        )
        ...
        completed_assignments = OrderAssignment.objects.filter(
            report=self.report, status="completed"
        )
    ...
    def test_cancel_report_updates_assignment_status(self):
        ...
        cancelled_assignments = OrderAssignment.objects.filter(
            report=self.report, status="cancelled"
        )
...
class DashboardTests(TestCase):
    # def __init__(self):
    #     super(DashboardTests, self).__init__()
    ...
    def test_call_report_submission(self):
        ...
        response = self.client.post(
            "/api/dashboard/call_report/", data, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        time.sleep(2)
        response = self.client.post(
    ...
    def test_change_duty_time(self):
        data = {"duty_time": "4"}
        logger.info("未登录状态下更改值班时间测试开始...")
        response = self.client.post(
            "/api/dashboard/change_duty_time/",
            data=data,
            content_type="application/json",
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        logger.info("未登录状态下更改值班时间测试通过")
        self.client.login(username="testuser", password="testpassword")
        logger.info("登录状态下以工作人员身份更改值班时间测试开始...")
        response = self.client.post(
            "/api/dashboard/change_duty_time/",
            data=data,
            content_type="application/json",
        ...
        logger.info("登录状态下以工作人员身份更改值班时间测试通过")
        self.client.logout()
        logger.info("登录状态下以管理员身份更改值班时间测试开始...")
        admin_user = User.objects.create_user(
            username="admin_test", password="admin123", email="admin_test@example.com")
        UserProfile.objects.create(
            user=admin_user, identity="admin", phoneNumber="13800000001", dutyTime="1")
        self.client.login(username="admin_test", password="admin123")
        response = self.client.post(
            "/api/dashboard/change_duty_time/",
            data=data,
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.client.logout()
        logger.info("登录状态下以管理员身份更改值班时间测试通过")
        logger.info("登录状态下以客户身份更改值班时间测试开始...")
        customer_user = User.objects.create_user(
            username="customer_test", password="customer123", email="customer_test@example.com")
        UserProfile.objects.create(
            user=customer_user, identity="customer", phoneNumber="13800000002")
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.client.logout()
        logger.info("登录状态下以客户身份更改值班时间测试通过")
