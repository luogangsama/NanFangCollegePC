...
def call_report(request):
    ...
    return JsonResponse(
        {"message": "Success", "orderDetails": "订单提交成功"}, status=200
    )
...
def user_get_history_report(request):
    ...
    report_infos = call_report_table.objects.all().order_by("-pk").filter(user=user)
    if len(report_infos) == 0:
        return JsonResponse({"message": "No history report"}, status=200)

    return_report_info = {"message": "Success", "report_info": []}
    # 获取用户历史订单信息
...
def worker_get_report_list(request):
    ...
    my_reports = (
        call_report_table.objects.all()
        .order_by("-pk")
        .filter(id__in=assignment_report_ids)
    )
    ...
    return JsonResponse({"message": "Success", "report_info": reports}, status=200)
...
def save_user_info(request):
    ...
    new_name = (
        new_name if new_name else get_user_from_sessionid(sessionid=sessionid).username
    )
    ...
    user_id_with_new_name = (
        User.objects.filter(username=new_name).first().pk
...
def get_phone_number(request):
    ...
    try:
        ...
        return JsonResponse(
            {"message": "Success", "phoneNumber": phoneNumber}, status=200
        )
...
def assign_order(request):
    ...
    existing_assignments = OrderAssignment.objects.filter(
        report_id=report_id_int, worker__in=workers, status="active"
    )
        ...
        with transaction.atomic():
            for worker in workers:
                OrderAssignment.objects.create(
                    report=report, worker=worker, assigned_by=user
                )
...
def get_staff_of_same_day(request):
    ...
    return_data = {"message": "Success", "workers": []}

    for staff in staffs:
        current_assignments = OrderAssignment.objects.filter(
            worker=staff.user, status="active"
        ).count()
...
def get_report_of_same_day(request):
    ...
    reports = (
        call_report_table.objects.all()
        .order_by("-pk")
        .filter(weekday=UserProfile.objects.get(user=user).dutyTime)
    )

    return_data = {"message": "Success", "reports": []}
...
class Change_duty_time(APIView):
    """
    修改值班时间
    """
    ...
    def post(self, request):
        user = request.user
        if user.profile.identity == "admin" or user.profile.identity == "worker":
            data = request.data
            ...
            return Response(
                {"message": "Permission error"}, status=status.HTTP_403_FORBIDDEN
            )
...
class Get_duty_time(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"message": "Success", "dutyTime": duty_time}, status=status.HTTP_200_OK
        )
